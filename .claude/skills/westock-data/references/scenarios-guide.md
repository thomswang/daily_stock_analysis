...
→ 一次批量（各 1 条；不要 kline/technical 按股拆开）：
   ...
   westock-data news list sh600519,sz000651 --limit 5
   westock-data fund north-holding sh600519,sz000651
→ 所属产业链（仅单股，可并行）：
   westock-data industry-chain sh600519
   westock-data industry-chain sz000651
```
...
> 命令：`news list/detail` `notice list/detail`
...
AI 步骤：
1. 搜索股票：westock-data search 贵州茅台 → sh600519
2. 查询新闻列表：westock-data news list sh600519 --limit 20
   → 解析新闻标题、来源、发布时间、摘要
3. 可选：查询市场资讯：`westock-data news list sh000001,sz399001,sz399006 --limit 20`（沪深指数）
   → 解析市场要闻、政策动向
...
用户："电子行业北向持仓分布如何？"
→ 若用户未给代码：westock-data search 电子 --type sector → 取 sw1 行业代码
...
用户："查看腾讯最近4期的综合损益表"
→ westock-data finance hk00700 --type income
```
