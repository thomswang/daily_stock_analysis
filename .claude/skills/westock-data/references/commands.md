...
> ⚙️ **示例约定**：下文 `westock-data <命令>` 为简写，实际执行 `node <本SKILL.md所在目录>/scripts/index.js <命令>`。
...
> 实时行情按市场差异化：A股含涨跌停价/每手/股息TTM；港股含每手/ADR；美股含盘前盘后/EPS TTM。`search`/`minute` 不支持多代码批量；`fund flow` 跨市场须分开查。完整列表见 [SKILL.md](../SKILL.md) 或 [routing-guide.md §六/§九](./routing-guide.md)。
...
```bash
westock-data news list sh600000 --limit 20     # 个股新闻
westock-data news list sh000001 --limit 20     # 指数资讯（如上证指数）
westock-data news list sh000001,sz399001,sz399006 --limit 20   # 沪深市场资讯
...
> - 港股：`hkHSI,hkHSTECH`；美股：`usDJI,usIXIC`
> - 单指数：如 `news list sh000001`
>
> 多指数批量按标的分段展示；热文榜用 `hot news`（另一数据源）。
...
```bash
westock-data finance sh600000                         # 默认三大表，最新1期
westock-data finance sh600519,sz000651 --num 4        # 批量三大表
...
```bash
westock-data industry-chain sh600519              # 查询单只股票所属产业链（不支持多代码）
```
