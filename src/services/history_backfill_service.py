...
2. **拉取（分两层、分两表）**：
   - 第 1 层：DataFetcherManager.get_daily_data() → stock_daily（kline 时间序列 OHLCV）
   - 第 2 层：DailyQuoteService quote --date 逐日循环 → stock_daily_quote（截面，含换手率）
...
class HistoryBackfillService:
    ...
    def quote_service(self):
        if self._quote_service is None:
            ...
            self._quote_service = DailyQuoteService(self.repo.db)
    ...
    def _backfill_one(
        ...
        # 第 2 层：quote --date 截面 → stock_daily_quote（对齐 westock-data/test runDailyK）
        # 采用「整个规划区间外包」一次性回填而非按 segments 循环：quote 表本身按 code+date
        # upsert 幂等，一次调用避免了「smart 前后两段中间日子漏拉」的边界问题，也减少了
        # 逐段 sleep 的等待开销。
        quote_rows = 0
        if _is_cn_a_share(code):
            ...
            try:
                quote_rows = self.quote_service.backfill_and_save(
                    code, start=quote_span_start, end=quote_span_end, overwrite=True,
                )
