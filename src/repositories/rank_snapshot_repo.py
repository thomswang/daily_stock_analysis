# -*- coding: utf-8 -*-
"""
===================================
横截面强弱打分快照 数据访问层
===================================

职责：
1. 按 as_of_date 幂等写入「全市场每只票的强弱分」快照
2. 查询某快照日（默认最新）的强弱榜，支持按行业过滤 + 取前 N
3. 列出某快照日的行业清单（供前端行业下拉）
4. 概览 / 最新快照日

写法对齐 stock_industry_repo：通过 DatabaseManager.get_session() 操作 ORM。
业务加工（分位、建议权重）放在 Service 层，本层只负责存取。
"""

from __future__ import annotations

import logging
from datetime import date, datetime
from typing import Any, Dict, List, Optional

from sqlalchemy import distinct, func, select
from sqlalchemy.dialects.sqlite import insert as sqlite_insert

from src.storage import DatabaseManager, StockRankSnapshot

logger = logging.getLogger(__name__)


class RankSnapshotRepository:
    """stock_rank_snapshot 表的数据访问层。"""

    def __init__(self, db_manager: Optional[DatabaseManager] = None):
        self.db = db_manager or DatabaseManager.get_instance()

    def save_snapshot(
        self,
        rows: List[Dict[str, Any]],
        *,
        as_of_date: date,
        model_name: str,
        model_version: Optional[str] = None,
    ) -> int:
        """把一批打分结果按 as_of_date 幂等写入。

        Args:
            rows: [{code, stock_name, industry, strength_score, last_close}, ...]
            as_of_date: 打分对应的行情日期
            model_name / model_version: 打分所用模型（可追溯）

        Returns:
            实际写入（新增+更新）的记录数
        """
        if not rows:
            return 0

        now = datetime.now()
        records: List[Dict[str, Any]] = []
        for r in rows:
            code = (r.get("code") or "").strip().upper()
            score = r.get("strength_score")
            if not code or score is None:
                continue
            records.append({
                "as_of_date": as_of_date,
                "code": code,
                "stock_name": r.get("stock_name"),
                "industry": (r.get("industry") or None),
                "strength_score": float(score),
                "last_close": (float(r["last_close"]) if r.get("last_close") is not None else None),
                "model_name": model_name,
                "model_version": model_version,
                "created_at": now,
            })
        if not records:
            return 0

        written = 0
        _CHUNK = 150
        with self.db.get_session() as session:
            for i in range(0, len(records), _CHUNK):
                chunk = records[i:i + _CHUNK]
                stmt = sqlite_insert(StockRankSnapshot).values(chunk)
                excluded = stmt.excluded
                session.execute(
                    stmt.on_conflict_do_update(
                        index_elements=["as_of_date", "code"],
                        set_={
                            "stock_name": excluded.stock_name,
                            "industry": excluded.industry,
                            "strength_score": excluded.strength_score,
                            "last_close": excluded.last_close,
                            "model_name": excluded.model_name,
                            "model_version": excluded.model_version,
                        },
                    )
                )
                written += len(chunk)
            session.commit()
        logger.info("强弱打分快照写入完成：%d 条，as_of=%s，model=%s", written, as_of_date, model_name)
        return written

    def latest_snapshot_date(self) -> Optional[date]:
        with self.db.get_session() as session:
            return session.execute(
                select(func.max(StockRankSnapshot.as_of_date))
            ).scalar_one_or_none()

    def delete_snapshot(self, as_of_date: date) -> int:
        """删除某快照日的全部记录（用于清理试跑/过期快照）。返回删除条数。"""
        from sqlalchemy import delete as sa_delete

        with self.db.get_session() as session:
            res = session.execute(
                sa_delete(StockRankSnapshot).where(StockRankSnapshot.as_of_date == as_of_date)
            )
            session.commit()
            return int(res.rowcount or 0)

    def get_ranking(
        self,
        *,
        industry: Optional[str] = None,
        top_n: Optional[int] = None,
        as_of_date: Optional[date] = None,
    ) -> List[Dict[str, Any]]:
        """读取某快照日（默认最新）的强弱榜，按分数降序；可按行业过滤、取前 N。

        Returns: [{code, stock_name, industry, strength_score, last_close}, ...]（强→弱）
        """
        target = as_of_date or self.latest_snapshot_date()
        if target is None:
            return []
        stmt = (
            select(
                StockRankSnapshot.code,
                StockRankSnapshot.stock_name,
                StockRankSnapshot.industry,
                StockRankSnapshot.strength_score,
                StockRankSnapshot.last_close,
            )
            .where(StockRankSnapshot.as_of_date == target)
            .order_by(StockRankSnapshot.strength_score.desc())
        )
        if industry:
            stmt = stmt.where(StockRankSnapshot.industry == industry)
        if top_n is not None and top_n > 0:
            stmt = stmt.limit(int(top_n))
        with self.db.get_session() as session:
            rows = session.execute(stmt).all()
        return [
            {
                "code": c, "stock_name": nm, "industry": ind,
                "strength_score": float(sc) if sc is not None else None,
                "last_close": float(lc) if lc is not None else None,
            }
            for c, nm, ind, sc, lc in rows
        ]

    def list_industries(self, as_of_date: Optional[date] = None) -> List[Dict[str, Any]]:
        """列出某快照日（默认最新）的行业清单及每个行业的股票数（按数量降序）。"""
        target = as_of_date or self.latest_snapshot_date()
        if target is None:
            return []
        with self.db.get_session() as session:
            rows = session.execute(
                select(StockRankSnapshot.industry, func.count())
                .where(
                    StockRankSnapshot.as_of_date == target,
                    StockRankSnapshot.industry.isnot(None),
                )
                .group_by(StockRankSnapshot.industry)
                .order_by(func.count().desc())
            ).all()
        return [{"industry": ind, "count": int(cnt)} for ind, cnt in rows if ind]

    def summary(self) -> Dict[str, Any]:
        """概览：最新快照日、覆盖股票数、行业数。"""
        latest = self.latest_snapshot_date()
        codes = industries = 0
        if latest is not None:
            with self.db.get_session() as session:
                codes = int(session.execute(
                    select(func.count()).where(StockRankSnapshot.as_of_date == latest)
                ).scalar_one() or 0)
                industries = int(session.execute(
                    select(func.count(distinct(StockRankSnapshot.industry)))
                    .where(StockRankSnapshot.as_of_date == latest)
                ).scalar_one() or 0)
        return {
            "latest": latest.isoformat() if latest else None,
            "latest_codes": codes,
            "latest_industries": industries,
        }
