...
- ratio(涨跌幅) / preClose(昨收)：westock kline 不返回这两项，按前一日 qfq 收盘价
  链式推导；首行取库内该 code 在 adj_type='qfq' 下早于本批的最近收盘价作种子。
...
class WestockOhlcvIngestor:
    ...
    def _build_df(self, code: str, raw: List[dict]):
        """raw kline → ohlcv DataFrame（含 volume×100、派生 preClose/ratio、time）。"""
        import pandas as pd
        ...
        # 链式推导 preClose / ratio（qfq 口径：前一日 qfq 收盘价）
